...
2. **(b) Pre-flight overlap detection at batch staging time.** A dedicated read-only endpoint, `POST /api/admin/bulk-import/preflight`, resolves every candidate `ScientificName` against GBIF `species/match` via the same `GbifDedupResolver` the runtime enrichment path uses (single source of truth — the pre-flight cannot drift from enrichment because it *is* enrichment's resolver, called earlier). The endpoint applies two cross-checks: (i) for each resolved accepted key, query `Plants` for any existing row carrying that key under a different `ScientificName`; (ii) detect intra-batch collisions where two candidates resolve to the same accepted key. The companion PowerShell client (`scripts/bulk-import/Invoke-BulkImportPreflight.ps1`) chunks the curated CSV, posts to the endpoint, and writes `exports/flagged-overlaps.csv` for human review. No auto-resolution — the curator decides per row whether to drop the duplicate, merge with the existing row, or accept the collision knowingly. Tracked under SMA-45.

3. **(c) Runtime resilience: catch `23505` on `IX_Plants_GbifTaxonKey`.** Even with (b), taxonomy drift means a previously-enriched plant can collide retroactively with a new plant. The GBIF enrichment controller (and the future shared chunk runner from Issue #83) catches the specific `PostgresException { SqlState: "23505", ConstraintName: "IX_Plants_GbifTaxonKey" }` case and classifies the outcome as `Skipped` with `Reason: "DuplicateTaxonKey"` instead of `Failed`. The driver's per-plant transaction still rolls back, but the batch continues and the operator gets a structured signal (which existing row owns the key) that a merge is needed. **Implemented by SMA-46 / PR #90** — the catch lives on `PlantTaxonomyController.Enrich` around `SaveChangesAsync`; when SMA-12 lands and consolidates the three controllers' EnrichAll loops, the catch moves into the shared runner with no contract change.
...
- **Layer (c) winning-plant SELECT in the 23505 catch becomes a hot path.** The catch does one `SELECT Id, ScientificName FROM "Plants" WHERE "GbifTaxonKey" = …` per collision (PR #90). At the projected 10-50 collisions per 1000 plants this is negligible on an already-slow exception path. If the SMA-13 scale-up reveals the collision rate is materially higher OR the lookup becomes a measurable fraction of EnrichAll wall-time, switch to a batch-wide pre-load (resolve all winning IDs in one query at chunk start) or drop the structured-log enrichment and accept a "Plant {id} resolves to GbifTaxonKey {key} (winner not resolved at log time)" message.
