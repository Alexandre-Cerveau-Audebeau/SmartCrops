...
describe('the figures that read the weather, when it is missing (arbitrage 4)', () => {
  ...
  // The weather the page hands its figures in each state: nothing while the
  // first load is out; nothing after ANY failure (round 2, É8) — the last
  // aggregate `useDashboardWeather` keeps across a failed refresh (E2 b of ③b)
  // names the stored place, and the page's one `displayWeather` never passes
  // it to a figure. The real hook and its kept aggregate are put to the test
  // on the page (`GardensDashboard.keyfigures.weather.test.tsx`).
  ...
  it('behind a failure: « indisponible » — and the last aggregate is not counted on', () => {
    // A failed first load and a failed refresh are ONE state for the band:
    // either way the page hands it no aggregate.
