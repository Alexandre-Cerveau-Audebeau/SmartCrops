...
export default function KeyFiguresOptionsPanel({ options, input, onChange }: Props) {
  ...
  // did, said once (the reorderable list hands its sentences here).
  ...
  const openCatalogue = (slot: number) => setCatalogue({ slot, choice: figures[slot]! });
  ...
  const liveRegion = (
    <Box
      ...
      sx={{
        ...noteSx,
        color: 'text.primary',
        ...(said && { backgroundColor: 'surfaceSubtle', borderRadius: '8px', p: '8px 10px' }),
      }}
    >
      {said}
    </Box>
      ...
      >
        ...
        <Typography component="h4" sx={{ m: 0, fontSize: DASHBOARD_TYPE.body, fontWeight: 700, lineHeight: 1.35 }}>
          {t('dashboard.blocks.keyfigures.panel.catalogueTitle', {
            figure: nameOf(figures[catalogue.slot]!),
            place: placeOf(catalogue.slot),
          })}
        </Typography>
        <RadioGroup
          aria-label={t('dashboard.blocks.keyfigures.panel.catalogueLabel')}
          value={catalogue.choice}
          onChange={(event) => setCatalogue({ ...catalogue, choice: event.target.value as KeyFigure })}
          sx={{ gap: '2px' }}
        >
              ...
              {members.map((figure) => {
                ...
                return (
                  <FormControlLabel
                    key={figure}
                    value={figure}
                    control={<Radio size="small" autoFocus={figure === catalogue.choice} />}
                    sx={{
                      m: 0,
                      alignItems: 'flex-start',
                      borderRadius: '8px',
                      pr: '6px',
                      ...(figure === catalogue.choice && { backgroundColor: tk.tint }),
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '10px', py: '4px' }}>
      ...
      <Button
        ...
        onClick={() => {
          ...
          setSaid(t('dashboard.blocks.keyfigures.panel.resetSaid'));
        }}
        sx={{
