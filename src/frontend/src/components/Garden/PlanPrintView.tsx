 ...
 * only thing: name, the meta line the page already computes, the date, the
 * read-only grid scaled to the printable width (day palette, layer per the
 * box), the legend, the plant list with quantities, a footer. Two frames
 * after mounting it calls window.print(); the browser's dialog produces the
 * PDF (« Enregistrer au format PDF ») — accepted deviation from a direct
 * download. `afterprint` (or the fallback timer) reports back so the page
 * unmounts the view.
 */
    ...
    <Portal>
      <ThemeProvider theme={LIGHT_THEME}>
        ...
        {/* The attribute is PLAN_PRINT_ROOT_ATTR — the stylesheet above hides
            every other <body> child on paper and this one on screen. */}
        <Box
          ...
          {plants.length > 0 && (
            <Box component="section" sx={{ mt: '16px' }}>
              ...
              >
                <TableHead>
                  <TableRow>
                    <TableCell>{t('planner.export.print.plant')}</TableCell>
                    <TableCell>
                ...
                <TableBody>
                  {plants.map((plant) => (
                    <TableRow key={plant.plantId} sx={{ breakInside: 'avoid' }}>
                      <TableCell>
                        <Box
                          component="span"
                          aria-hidden
                      ...
                      <TableCell sx={{ fontStyle: 'italic', color: tk.tSci }}>
