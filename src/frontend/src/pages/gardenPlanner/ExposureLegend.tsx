 ...
 * swatch (5.4, SMA-15) appears only when a blocking structure exists — §9
 * hatches BOTH Ombre and Ombre portée, so the swatch shares the shade
 * fill/border + hatch and differs by its §13 label. DnD swatches ship with
 * chantier E.
 */
export const ExposureLegend = memo(function ExposureLegend({
  ...
  return (
    ...
    >
      ...
      {hasCastShadow && (
        <Box
          data-testid="legend-cast-shadow"
          sx={{ display: 'flex', alignItems: 'center', gap: '7px' }}
        >
          <Box
            sx={{
              ...
              height: { xs: 13, sm: 16 },
              borderRadius: '5px',
              bgcolor: tk.expo.shade.fill,
              border: `1px solid ${tk.expo.shade.border}`,
              backgroundImage: tk.hatch,
            }}
          />
          <Typography
            sx={{
