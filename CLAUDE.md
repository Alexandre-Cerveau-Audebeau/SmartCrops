...
- **API** host port: 5000.
- **Verification suite** (before every commit): `dotnet build SmartCrops.sln && dotnet test SmartCrops.sln && dotnet format SmartCrops.sln --verify-no-changes && cd src/frontend && npm run lint && npm test -- --testTimeout=20000`.
- **Frontend tests**: run vitest with `--testTimeout=20000`. The default 5000 ms flakes `PlantLibrary.test.tsx` under load (SMA-174).
